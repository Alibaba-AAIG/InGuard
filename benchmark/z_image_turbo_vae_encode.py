"""
VAE encoding script: compress images into latents and save them using the Z-Image-Turbo VAE Encoder.
...
Normalization formula (encode → latent_x1):
    latent_x1 = (vae.encode(img).sample() - shift_factor) * scaling_factor

Inverse decoding formula (for verification):
    img = vae.decode(latent_x1 / scaling_factor + shift_factor)

Usage:
    python z_image_turbo_vae_encode.py \
...
def load_vae(model_path, device):
    """Load the VAE part of ZImagePipeline, releasing the non-VAE components afterwards to save memory"""
    from diffusers import ZImagePipeline

    print(f"Loading ZImagePipeline (VAE only) from {model_path} ...")
    ...
    return vae, image_processor, scaling_factor, shift_factor


def encode_image(image_path, vae, image_processor, device,
                 scaling_factor, shift_factor, target_height, target_width):
    """
...
def main():
    parser = argparse.ArgumentParser(description="Z-Image-Turbo VAE Encode: image → latent (aligned with the latents_x1 distribution)")
    ...
    parser.add_argument("--model_path", required=True,
                        help="ZImagePipeline model path (a local dir or a HuggingFace repo id, auto-downloaded in the latter case)")
    ...
    vae, image_processor, scaling_factor, shift_factor = load_vae(args.model_path, args.device)
