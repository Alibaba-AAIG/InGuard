"""
VAE encoding script: compress images into latents and save them using the HunyuanImage 2.1 VAE Encoder.

The output latent distribution matches the latents_x1 saved during diffusion inference.

latents_x1 storage format: [B, C, H, W] (4D)
Normalization formula (encode → latent_x1):
    latent_x1 = vae.encode(img).sample() * scaling_factor

Inverse decoding formula (for verification):
    img = vae.decode(latent_x1 / scaling_factor)

Usage:
    python hunyuan_image_2_1_vae_encode.py \
        --input_dir /path/to/images \
        --output_dir /path/to/output \
        --model_path /path/to/HunyuanImage-2.1-Diffusers \
        --device cuda:0
"""
import os
import argparse
...
def load_vae(model_path, device):
    """Load the VAE part of HunyuanImagePipeline, releasing the non-VAE components afterwards to save memory"""
    from diffusers import HunyuanImagePipeline

    print(f"Loading HunyuanImagePipeline (VAE only) from {model_path} ...")
    ...
    spatial_compression_ratio = pipe.vae.config.spatial_compression_ratio
    ...
    print(f"  spatial_compression_ratio={spatial_compression_ratio}")
    return vae, image_processor, scaling_factor


def encode_image(image_path, vae, image_processor, device,
                 scaling_factor, target_height, target_width):
    """
    ...
    with torch.no_grad():
        ...
        latent = latent_dist.sample()  # [1, C, H_latent, W_latent]
...
def main():
    parser = argparse.ArgumentParser(description="HunyuanImage 2.1 VAE Encode: image → latent (aligned with the latents_x1 distribution)")
    ...
    parser.add_argument("--model_path", required=True,
                        help="HunyuanImagePipeline model path (a local dir or a HuggingFace repo id, auto-downloaded in the latter case)")
    ...
    vae, image_processor, scaling_factor = load_vae(args.model_path, args.device)
